...
function approvePendingApproval(state, approvalId, context = {}) {
  ...
  if (context.sessionId && pendingApproval.sessionId && pendingApproval.sessionId !== context.sessionId) {
    return { success: false, reason: 'The approval request does not match the initiating session.' };
  }
  if (context.actionKey && pendingApproval.actionKey !== context.actionKey) {
