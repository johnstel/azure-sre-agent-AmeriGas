...
- The incident evidence-pack export buttons also work remotely: instead of a plain navigation/`window.open()` (which cannot attach the auth header either), `public/incident-export.js` fetches the export through the same authenticated `apiClient.request()` used everywhere else, reads the response as a `Blob`, and triggers the download via a short-lived `URL.createObjectURL()` anchor that is revoked immediately afterward. The token is never placed in a URL, the DOM, a log, or the exported (already server-redacted) content itself.
