...
| "Which pods in the propane namespace are not in a Running state?" | Quick status check across all workloads |
| "Show me all warning and error events in the propane namespace from the last 30 minutes" | Event-level triage when you know something recently broke |
...
| "What's the exit code and termination reason for the last failed container in tank-monitor?" | Precise failure details |
| "Are any init containers failing across my deployments?" | Init container issues are easy to miss |
| "Compare the running pod spec for tank-monitor to its deployment spec — are there any drift issues?" | Detect manual overrides or config drift |

### Networking Issues

| Prompt | When to Use |
|--------|-------------|
| "Can customer-portal reach tank-monitor on port 3000?" | Validate service-to-service connectivity |
| "Are there any network policies blocking traffic in the propane namespace?" | Surface overly restrictive policies |
| "Why does the tank-monitor Service have zero endpoints?" | Diagnose selector/label mismatches |
| "Show me DNS resolution results for mongodb inside the cluster" | Troubleshoot service discovery |
| "Trace the network path from the customer-portal ingress to the backend services" | End-to-end connectivity map |
...
| Prompt | When to Use |
|--------|-------------|
| "What's the current CPU and memory utilization for each pod in the propane namespace?" | Real-time resource snapshot |
...
| Prompt | When to Use |
|--------|-------------|
| "Query the last 50 error-level logs from the tank-monitor container" | Targeted log retrieval |
...
| "Would scaling inventory-service to 5 replicas fit on the current nodes?" | Pre-scale feasibility check |
...
| Prompt | When to Use |
|--------|-------------|
| "Are any containers running as root in the propane namespace?" | Security posture check |
...
| "Show me the RBAC roles and bindings in the propane namespace" | Access control review |
...
| "Check if network policies exist for all services in the propane namespace" | Network segmentation audit |
...
| "Show me the rollout history for tank-monitor" | Deployment version tracking |
| "Is the latest rollout for inventory-service complete and healthy?" | Rollout status check |
| "What image versions are running vs. what's defined in the deployment spec?" | Image version drift |
| "Compare the current tank-monitor deployment to the previous revision" | Diff between revisions |
...
| Prompt | When to Use |
|--------|-------------|
| "Map out the service dependencies in the propane namespace" | Service topology discovery |
...
| Prompt | What It Does |
|--------|-------------|
| "Restart the tank-monitor pods" | Rolling restart of a deployment |
| "Scale inventory-service to 3 replicas" | Horizontal scaling |
| "Delete the demand-forecast-overload deployment" | Remove rogue/test workloads |
| "Remove the deny-tank-monitor network policy" | Unblock network traffic |
| "Scale the mongodb deployment back to 1 replica" | Restore a dependency |
| "Roll back the tank-monitor deployment to the previous revision" | Deployment rollback |
| "Cordon the unhealthy node and drain its workloads" | Node maintenance |
| "Increase the memory limit for tank-monitor to 256Mi" | Resource limit adjustment |
...
| "Monitor pod restarts in propane namespace — alert if any pod restarts more than 3 times in an hour" | Every 15 min | Restart anomaly detection |
...
| Prompt | Why It Works |
|--------|-------------|
| "The customer portal is broken" | SRE Agent discovers what "broken" means |
| "Customers complaining deliveries aren't being tracked" | Business-language → technical diagnosis |
| "We deployed something an hour ago and now things are bad" | Change correlation from a human perspective |
| "Is my propane platform ready for peak winter heating season?" | Capacity + health in business terms |
...
1. **Layer your prompts** — Start broad ("what's wrong with propane services?"), then narrow ("why is tank-monitor failing?"), then act ("restart tank-monitor")
2. **Use business language** — SRE Agent translates "deliveries aren't being tracked" into pod metrics and traces
