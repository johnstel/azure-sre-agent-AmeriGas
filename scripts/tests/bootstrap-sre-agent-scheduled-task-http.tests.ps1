...
<#
.SYNOPSIS
    Execution-level HTTP test for the REAL scripts/bootstrap-sre-agent-scheduled-task.ps1
    Invoke-DataPlaneRequest function — no mocking of that function.

.DESCRIPTION
    scripts/tests/bootstrap-sre-agent-scheduled-task.tests.ps1 mocks
    Invoke-DataPlaneRequest everywhere it needs a canned HTTP response. This
    file closes that gap, mirroring the pattern in
    scripts/tests/bootstrap-sre-agent-response-plan-http.tests.ps1: it
    stands up a real System.Net.HttpListener on loopback, runs the
    unmodified Invoke-DataPlaneRequest in a background job, and inspects
    the request the listener actually received:
      - The Authorization header carries the exact supplied test token as
        "******", for GET, PUT, PATCH, POST (RunNow), and DELETE.
      - PUT/PATCH/POST request bodies are sent as UTF-8 WITHOUT a
        byte-order-mark and with Content-Type: application/json.
      - All methods send Accept: application/json.
      - 404/405/500 status codes are correctly classified without throwing.
    ...
    This is a real network round-trip (127.0.0.1 only), not a mock.

.EXAMPLE
    Invoke-Pester -Path scripts/tests/bootstrap-sre-agent-scheduled-task-http.tests.ps1
#>
...
BeforeAll {
    ...
    function Get-EphemeralLoopbackListener {
        ...
        to 127.0.0.1, plus the port it bound to. Uses a distinct port range
        from both the knowledge-bootstrap and response-plan HTTP test files
        to avoid any collision if all three happen to run concurrently.
        #>
        [CmdletBinding()]
        param()

        $candidatePorts = 18900..18960
...
Describe "Invoke-DataPlaneRequest — real HTTP execution (no mocking)" {
    Context "GET request against the scheduled-task path" {
        It "sends 'Authorization: ******' and 'Accept: application/json', and never leaks the token" {
            ...
            try {
                ...
                $job = Invoke-RealDataPlaneRequestInJob -Port $port -Token $testToken -Method 'GET' -Path '/api/v2/extendedAgent/scheduledtasks/daily-propane-health-report'
                ...
                $responseBytes = [System.Text.Encoding]::UTF8.GetBytes('{"name":"daily-propane-health-report"}')
                ...
                $functionResult.Content.name | Should -Be 'daily-propane-health-report'
    ...
    Context "PUT request with a JSON body (create)" {
        It "sends the body as UTF-8 WITHOUT a byte-order-mark, Content-Type: application/json, and the exact Authorization header" {
            ...
            $bodyJson = "{`"name`":`"daily-propane-health-report-$uniqueMarker`",`"frequency`":`"Daily`",`"agentAutonomyLevel`":`"Autonomous`"}"
            ...
            try {
                $acceptTask = $listener.GetContextAsync()
                $job = Invoke-RealDataPlaneRequestInJob -Port $port -Token $testToken -Method 'PUT' -Path '/api/v2/extendedAgent/scheduledtasks/daily-propane-health-report' -BodyJson $bodyJson
    ...
    Context "PATCH request with a JSON body (update)" {
        It "sends PATCH with a no-BOM UTF-8 body and the exact Authorization header" {
            $testToken = 'exec-test-token-' + [guid]::NewGuid().ToString('N')
            ...
            try {
                $acceptTask = $listener.GetContextAsync()
                $job = Invoke-RealDataPlaneRequestInJob -Port $port -Token $testToken -Method 'PATCH' -Path '/api/v2/extendedAgent/scheduledtasks/daily-propane-health-report' -BodyJson $bodyJson
                ...
                $responseBytes = [System.Text.Encoding]::UTF8.GetBytes('{"enabled":false}')
                $context.Response.StatusCode = 200
                $context.Response.ContentType = 'application/json'
                $context.Response.ContentLength64 = $responseBytes.Length
                $context.Response.OutputStream.Write($responseBytes, 0, $responseBytes.Length)
    ...
    Context "POST request with no body (RunNow candidate trigger)" {
        It "sends 'Authorization: ******' and 'Accept: application/json' with no body" {
            ...
            try {
                $acceptTask = $listener.GetContextAsync()
                $job = Invoke-RealDataPlaneRequestInJob -Port $port -Token $testToken -Method 'POST' -Path '/api/v2/extendedAgent/scheduledtasks/daily-propane-health-report/run'
                ...
                $responseBytes = [System.Text.Encoding]::UTF8.GetBytes('{"threadId":"THREAD-abc"}')
                ...
                $functionResult.Success | Should -Be $true
                $functionResult.Content.threadId | Should -Be 'THREAD-abc'
    ...
    Context "DELETE request" {
        It "sends 'Authorization: ******' with no body" {
            ...
            try {
                $acceptTask = $listener.GetContextAsync()
                $job = Invoke-RealDataPlaneRequestInJob -Port $port -Token $testToken -Method 'DELETE' -Path '/api/v2/extendedAgent/scheduledtasks/daily-propane-health-report'
    ...
    Context "Capability-detection failure responses (404/405) are correctly classified, never crash" {
        ...
        It "returns Success=false and StatusCode=405 for the RunNow candidate path, without throwing" {
            $testToken = 'exec-test-token-' + [guid]::NewGuid().ToString('N')
            $bound = Get-EphemeralLoopbackListener
            $listener = $bound.Listener
            $port = $bound.Port
            ...
            try {
                ...
                $functionResult.StatusCode | Should -Be 405
                $functionResult.Success | Should -Be $false
            }
            finally {
                $listener.Stop()
        ...
        It "returns Success=false and StatusCode=500 for a genuine server error, without throwing" {
            $testToken = 'exec-test-token-' + [guid]::NewGuid().ToString('N')
            $bound = Get-EphemeralLoopbackListener
            $listener = $bound.Listener
            $port = $bound.Port
